...
第一次打开包含本次打包时旧版 `data/project.json` 的内容，之后优先恢复当前浏览器的草稿。原始示例保存在 `examples/签到系统_使用示例.sysplan`。点击“新建”开始自己的系统案。
...
打开示例会替换当前编辑项目，可撤回恢复；自己的工作请先保存项目文件。示例文件也保存在 `examples/交互与状态_使用示例.sysplan`。
...
页面关闭后无需再“停止工具”。旧的 `data/`、`exports/` 和可选服务端保留，供恢复原项目和后续开发使用。
